...
test('persona center is routed, editable, explainable, and localized', () => {
  ...
  assert.match(personaCenterPageSource, /persona\.viewEvidence/);
  assert.match(personaCenterPageSource, /persona\.inferenceBadge/);
  assert.match(personaCenterPageSource, /persona\.acceptedBadge/);
  assert.match(personaCenterPageSource, /persona\.hiddenRecords/);
  assert.match(personaCenterPageSource, /persona\.restore/);
  assert.match(personaCenterPageSource, /persona\.deletePermanently/);
  assert.match(personaCenterPageSource, /persona\.deleteProfile/);
  assert.match(personaCenterPageSource, /persona\.memoryDisabledState/);
  assert.match(personaCenterPageSource, /selectedEvidence/);
  assert.match(personaCenterPageSource, /isHiddenRecordsOpen/);
  assert.match(personaCenterPageSource, /hidden\?: PersonaHiddenRecords/);
  assert.match(personaCenterPageSource, /hiddenRecordCount/);
  assert.match(personaCenterPageSource, /restoreHiddenRecord/);
  assert.match(personaCenterPageSource, /deleteHiddenRecord/);
  assert.match(personaCenterPageSource, /api\.delete\('\/persona\/profile'\)/);
  assert.match(personaCenterPageSource, /api\.delete\(`\/persona\/interests\/\$\{recordId\}`\)/);
  assert.match(personaCenterPageSource, /api\.delete\(`\/persona\/observations\/\$\{recordId\}`\)/);
  assert.match(personaCenterPageSource, /api\.delete\(`\/persona\/suggestions\/\$\{recordId\}`\)/);
  assert.match(personaCenterPageSource, /memory_enabled/);
  assert.match(personaCenterPageSource, /editedProfile/);
  assert.match(personaCenterPageSource, /status:\s*'active' \| 'accepted' \| 'hidden' \| 'rejected'/);
  assert.match(personaCenterPageSource, /applySavedProfileToCenter/);
  assert.match(personaCenterPageSource, /applyInterestStatusToCenter/);
  assert.match(personaCenterPageSource, /interest\.status === 'accepted'/);
  assert.match(personaCenterPageSource, /profile\.memory_enabled === false/);
  assert.match(personaCenterPageSource, /observations:\s*\[\]/);
  assert.match(personaCenterPageSource, /interests:\s*\[\]/);
  assert.match(personaCenterPageSource, /suggestions:\s*\[\]/);
  ...
  for (const localeFile of localeFiles) {
    ...
    assert.ok(locale.persona?.viewEvidence, `${localeFile.locale}.json needs persona.viewEvidence`);
    assert.ok(locale.persona?.inferenceBadge, `${localeFile.locale}.json needs persona.inferenceBadge`);
    assert.ok(locale.persona?.acceptedBadge, `${localeFile.locale}.json needs persona.acceptedBadge`);
    assert.ok(locale.persona?.hiddenRecords, `${localeFile.locale}.json needs persona.hiddenRecords`);
    assert.ok(locale.persona?.hiddenRecordsTitle, `${localeFile.locale}.json needs persona.hiddenRecordsTitle`);
    assert.ok(locale.persona?.restore, `${localeFile.locale}.json needs persona.restore`);
    assert.ok(locale.persona?.restored, `${localeFile.locale}.json needs persona.restored`);
    assert.ok(locale.persona?.deletePermanently, `${localeFile.locale}.json needs persona.deletePermanently`);
    assert.ok(locale.persona?.deleteProfile, `${localeFile.locale}.json needs persona.deleteProfile`);
    assert.ok(locale.persona?.deleteProfileConfirm, `${localeFile.locale}.json needs persona.deleteProfileConfirm`);
    assert.ok(locale.persona?.deleted, `${localeFile.locale}.json needs persona.deleted`);
    assert.ok(locale.persona?.evidenceModalTitle, `${localeFile.locale}.json needs persona.evidenceModalTitle`);
    assert.ok(locale.persona?.memoryDisabledState, `${localeFile.locale}.json needs persona.memoryDisabledState`);
